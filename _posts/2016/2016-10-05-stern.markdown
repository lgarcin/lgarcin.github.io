...
Supposons qu'un tel couple existe et posons $r=\frac{p}{q}$, $\rho=\frac{a}{b}$ et $\sigma=\frac{c}{d}$ (les fractions étant irréductibles). Puisque $\Delta(\rho,\sigma)=bc-ad=1$, $b\geq1$ et $c\geq1$. Par ailleurs, $p=a+c$ et $q=b+d$ donc $0\leq a\leq p-1$ et $0\leq d\leq q-1$. Enfin,
<span class="formula">
$$
\begin{align*}
...
de sorte que $bp\equiv1[q]$ et $cq\equiv1[p]$. Ainsi $b$ est l'inverse de $p$ modulo $q$ et $c$ est l'inverse de $q$ modulo $p$[^invmodulo] (les inverses sont bien définis car $p$ et $q$ sont premiers entre eux et sont uniques). Les conditions $bp-aq=1$ et $cq-dp=1$ définissent alors également $a$ et $d$ de manière unique. Réciproquement, notons $b$ l'inverse de $p$ modulo $q$ et $c$ l'inverse de $q$ modulo $p$. Alors $a=\frac{bp-1}{q}$ et $d=\frac{cq-1}{p}$ sont bien des entiers. On pose $\rho=\frac{a}{b}$ et $\sigma=\frac{c}{d}$. On a alors
<span class="formula">
$$
...
On peut remarquer que chacune des suites $u_n$ est strictement croissante. Il suffit d'utiliser de raisonner par récurrence sur $n$ en utilisant le résultat du premier lemme stipulant que la médiante de deux fractions distinctes est strictement comprise entre ces deux fractions.
